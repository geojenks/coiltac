...
%% wire cross sections (2 circular wires)

wire_radius = 75;
position_wire = 100;
wire_1_centre = [mpx-position_wire, mpy];
wire_2_centre = [mpx+position_wire, mpy];
err_start = length(error);
error = [error zeros(1,Ni)];
perc = 0;
for z = 1:Ni    % Number of iterations
    ...
    for i=2:Nx-1
        for j=2:Ny-1   
            ...
            if (i-(wire_1_centre(1)))^2 + (j-(wire_1_centre(2)))^2 < wire_radius^2
                V(i,j) = 100;
            elseif (i-(wire_2_centre(1)))^2 + (j-(wire_2_centre(2)))^2 < wire_radius^2
                V(i,j) = -100;
            else
    ...
    if mod(z,check_every) == 0
        ...
        perm = .00000000000885;
        Volt=200;
        Field = mean(nonzeros(elements_within_radius(E,[mpy,mpx-position_wire],[wire_radius, wire_radius+1])));
        %figure;plot(nonzeros(results))
        %mean(nonzeros(results))
        density = perm * Field;
        Q = density * 2*pi*wire_radius;
        numericalCap = Q/Volt;
        analyticalCap = perm*pi/(acosh((2*position_wire)/(2*wire_radius)));
...
%% wire cross sections (2 wires, side on)
%{
wire_radius = 5;
position_wire = 10;
wire_1_centre = [mpx-position_wire, mpy];
...
for z = 1:Ni    % Number of iterations
    for i=2:Nx-1
        for j=2:Ny-1   
            % The next two lines are to force the matrix to hold the 
            % potential values for all iterations
            if (i-(wire_1_centre(1)))^2 < wire_radius^2
                V(i,j) = 100;
            elseif (i-(wire_2_centre(1)))^2 < wire_radius^2 && j < 25
                V(i,j) = -100;
            else
                V(i,j)=0.25*(V(i+1,j)+V(i-1,j)+V(i,j+1)+V(i,j-1));
            end
